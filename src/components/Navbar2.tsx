        ...
        <div className='absolute z-10 top-0 left-0'>
            ...
            >
                <div className="flex flex-col items-center mb-20">
                    ...
                    <div style={!isHovered ? showStyle : hideStyle} className="flex flex-col text-3xl absolute">
                    ...
                    <div className='flex flex-col space-y-12'>
                        {icons.map((icon, index) => (
                            <Link key={index} href={icon.path} legacyBehavior>
                                <a className={`flex items-center w-full ${pathname === icon.path ? 'active' : ''}`}>
                                    <img
                                        className={`w-5 h-5`}
                                        style={{ filter: pathname === icon.path ? 'brightness(120%)' : 'none' }} // Adjusted for visibility
                                        src={`/${icon.name}.png`}
                                        alt={icon.label}
                                    />
                                    <span style={{ transition: 'opacity 0.7s ease-in-out', opacity: isHovered ? 1 : 0, visibility: isHovered ? 'visible' : 'hidden' }} className="ml-5">{icon.label}</span>
                                </a>
                            </Link>
